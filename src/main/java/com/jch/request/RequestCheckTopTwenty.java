...
public class RequestCheckTopTwenty {
	...
	public String request() {
		...
		try {
			...
			String urlWithParams = ksa.getKsaUrl() + ksa.getApiChkTopTwnUrl();
			...
			HttpRequest request = HttpRequest.newBuilder().uri(new URI(urlWithParams))
					...
					.header("appsecret", ksa.getAppSecret()).header("tr_id", ksa.getTrIdCTT()) // TR_ID 추가
					.GET().build();
			...
			if (response.statusCode() == 200) {
				...
				LoggerUtil.logWarning("Failed to get top 20 stock data. Status code: " + response.statusCode(),
						requestLogId);
				return "Failed";
			}
		} catch (Exception e) {
